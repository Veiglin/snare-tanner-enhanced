...
- **Honeylinks**: This framework includes a feature to configure and generate honeylinks. When a honeylink is accessed, it triggers an alert by sending a webhook request containing detailed information, such as the source IP, geographic location, and whether the IP is a known Tor exit node.
- **Bait Files**: This framework creates bait files together with the honeytokens, which mimic files that could be exploited. These files are strategically placed to lure the attacker.
...
HONEYTOKEN:
  ...
  WEBHOOK-URL: https://webhook.site/1234 # Only for local deployment
