...
Honeytokens are decoy files designed to detect unauthorized access. This framework integrates with [Canarytoken](https://canarytokens.org/nest/generate) to generate honeytokens in various formats, such as `docx`, `xlsx`, and `pdf`. These files are enriched with content generated by Large Language Models (LLMs) to make them appear realistic and enticing to attackers. When a honeytoken is accessed, it triggers a webhook alert, providing detailed information about the event, including the time, source IP, and other metadata. This feature ensures that any unauthorized access to sensitive-looking files is immediately detected and logged.
